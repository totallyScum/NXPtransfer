...
public class AutomaticBarcodeActivity extends AppCompatActivity implements BarcodeReader.BarcodeListener,
    ...
    //            Log.d("barCodeDataDDDDD",mainViewModel.getFragmentID()+"");\
         //       Bundle bundle=getIntent().getExtras();
       //         String user=bundle.getString("checkID");
          //      Log.d("barCodeDataDDDDD",intent2.getIntExtra("checkID",-1)+"");
    //            if (MainActivity.get)
                if (MainFragment.check==false)         //如果不是开票扫码
                {
                    ...
                    MainFragment.setCheck(false);
                    MainFragment.setIsFixed(true);
                    startActivity(intent);
                    finish();
                }else{
                    mainViewModel.setFragmentID(intent2.getIntExtra("checkID",0));        //开票扫码
                    Intent intent = new Intent(getApplicationContext(), MainActivity.class);
                    intent.putExtra("checkID",mainViewModel.getFragmentID());
                    intent.putExtra("barCodeData",barCodeData);
                    MainFragment.setCheck(true);
                    MainFragment.setIsFixed(false);
                    startActivity(intent);
                    finish();
                }
            }
